...
void prob2(void)
{
	...
	for (i = 0; i <= 360; i += 10) // 0 - 360도까지 10도 간격으로
	{
		...
		// 정수 좌표로 변환 (Value : -1 ~ 1 <-> 정수좌표 : -25 ~ 25)
	...
	for (i = 0; i <= 360; i += 10)
	{
		...
		// tan함수 값의 범위가 크므로 전과 다르게 200으로 나눠줌
...
void prob4(void)
{
	...
	if (max_count == 0)
		...
		printf("입력된 문자열에서 최대 회문의 길이는 %d 입니다.", max_count);
}
...
void prob6(void)
{
	...
	for (i = 0; i <= rank; i++) // 
	{
		...
		{
			printf("%c", ch - rank + j);
			// a(A)부터 알파벳 순서대로 / 각 줄마다 더 낮은 순위 알파벳만 출력
		}
		printf("\n");
	}
}
...
void probD(void)
{
	printf("*** 실행 가능한 함수들의 목록 *** \n");
	printf("==========================================\n");
	printf("1) ASCII 코드표를 출력하기 \n"); // prob3
	printf("2) 문자열에서 회문 판별 및 회문 최대길이 구하기 \n"); // prob4
	printf("3) 두 수의 최대공약수 구하기 \n"); // prob5
	printf("4) Swap 함수 - 두 변수의 값을 바꾸어 출력하기 \n"); // prob7
	printf("5) 정수들을 입력받아 오름차순 정렬하여 출력하기 \n"); // prob8
	printf("0) 종료 \n");
	printf("==========================================\n");
